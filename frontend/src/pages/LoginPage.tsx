...
export const LoginPage = () => {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-mesh px-6 relative overflow-hidden font-cairo">
      {/* 1. Background animated blobs - Fluid Mesh Feel */}
      ...
      <div className="w-full max-w-md bg-slate-50/70 backdrop-blur-3xl rounded-[2rem] p-8 md:p-10 relative z-10 shadow-[0_20px_50px_rgba(108,93,211,0.15)] border border-white/60">
        <div className="flex flex-col items-center mb-10">
          ...
          <h1 className="text-2xl font-black text-slate-900 tracking-tight text-center mb-1.5 Arabic-text-scale">
            {t('auth.sign_in_title')}
          </h1>
          <p className="text-slate-500 font-medium text-center px-4 leading-relaxed text-sm">
            {t('auth.sign_in_subtitle')}
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          {/* 3. Input Fields: bg-slate-50 with purple focus ring */}
          <div className="space-y-2">
            <label className="text-[13px] font-black text-slate-600 uppercase tracking-widest ml-1 block">
              {t('auth.email')}
            </label>
            <input
              type="email"
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4.5 text-slate-900 placeholder-slate-400 outline-none transition-all duration-300 focus:border-[#6C5DD3] focus:ring-4 focus:ring-purple-100"
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4.5 text-slate-900 placeholder-slate-400 outline-none transition-all duration-300 focus:border-[#6C5DD3] focus:ring-4 focus:ring-purple-100 pr-14"
                placeholder="••••••••"
                required
              />
              <button
          ...
          {error && (
            <div className="bg-rose-50 border border-rose-100 rounded-xl p-3 animate-shake">
              <p className="text-[13px] font-bold text-rose-600 text-center">{error}</p>
            </div>
          )}

          {/* 4. The Button: Gradient and specific hover effects */}
          <button
            type="submit"
            disabled={isLoading}
          ...
          >
            {isLoading ? (
              ...
              <>
                <span className="text-base">{t('auth.sign_in_button')}</span>
